...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    if args.resume:
        ...
        if 'pos_embed' in checkpoint['models'] and checkpoint['models']['pos_embed'].shape != \
                model_without_ddp.state_dict()['pos_embed'].shape:
            print(f"Removing key pos_embed from pretrained checkpoint")
            del checkpoint['models']['pos_embed']

        if 'decoder_pos_embed' in checkpoint['models'] and checkpoint['models']['decoder_pos_embed'].shape != \
                model_without_ddp.state_dict()['decoder_pos_embed'].shape:
            print(f"Removing key decoder_pos_embed from pretrained checkpoint")
            del checkpoint['models']['decoder_pos_embed']
...
def sliding_window(image, window_size=(384, 384), stride=128):
    ...
    h, w, _ = image.shape
    assert h == 384, "FSC_147 assume image height is 384."
    patches = []
    intervals = []
